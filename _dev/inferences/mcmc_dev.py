...
class MCMCABC(ABCBase):
    ...
    def _draw_first_posterior_sample(self, seed):
        ...
        while sample is None:
            ...
            sim_sumstat = self._stat_calc(sim)
            self._n_sims += 1
            distance = self.distance(self._obs_sumstat, sim_sumstat)
            if distance <= self._epsilon:
                sample = thetas

        return sample

    def _draw_from_proposal(self, scaling, next_gen):
...
if __name__ == "__main__":
    ...
    # initialize sampler
    sampler = MCMCABC(obs_data, simulator, summary_calculator, priors)
